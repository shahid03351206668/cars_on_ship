...
import { PlusCircle, ArrowUpRight, Clock, CreditCard, ChevronRight, Loader } from 'lucide-react'
...
function CreditCardUI({ card }: { card: SavedCard }) {
  return (
    <div className="relative w-56 p-4 overflow-hidden text-white shadow-lg select-none h-36 rounded-2xl bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-700">
      ...
      <p className="text-xs tracking-[0.18em] mb-3">•••• •••• •••• {card.last4}</p>
...
export default function WalletPayments() {
  ...
  const handleAddBalance = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/method/cars_on_ship.stripe.create_checkout_session', {
        method: 'POST',
        headers: {
      ...
      if (data.message && data.message.url) {
        ...
        throw new Error(data.exception || "Failed to create session")
      }
    } catch (err) {
            ...
            <div className="flex flex-wrap items-center gap-6">
              ...
              {!showAmountInput ? (
                <button
                  ...
                  className="flex flex-col items-center justify-center w-40 gap-2 text-gray-400 transition-colors border-2 border-gray-200 border-dashed h-36 rounded-2xl hover:border-orange-400 hover:text-orange-500"
                >
                ...
                <div className="flex flex-col items-center justify-center w-40 gap-3 p-4 border-2 border-orange-400 h-36 rounded-2xl bg-orange-50">
                  <input
                    type="number"
                    value={amount}
                    ...
                    placeholder="0.00"
                    step="0.01"
                    className="w-full px-2 py-1 border border-orange-300 rounded-md text-center text-sm focus:outline-none focus:ring-1 focus:ring-orange-500"
                  />
                  <button
                    onClick={handleAddBalance}
                    disabled={loading || !amount || parseFloat(amount) <= 0}
                    className="flex items-center justify-center w-full gap-2 py-2 text-white transition-colors bg-orange-500 rounded-lg hover:bg-orange-600 disabled:opacity-50"
                  >
                    {loading ? <Loader className="w-4 h-4 animate-spin" /> : <PlusCircle className="w-4 h-4" />}
                    <span className="text-[11px] font-bold uppercase">Pay Now</span>
                  </button>
                  <button
                    onClick={() => { setShowAmountInput(false); setAmount(''); }}
        ...
        <div className="p-5 bg-white border border-gray-100 shadow-sm rounded-xl">
          ...
          <div className="space-y-3">
            {ACTIVE_BIDS.map((bid) => (
              ...
              >
                <img src={bid.imageUrl} alt={bid.carName} className="object-cover w-20 border border-gray-200 rounded-md h-14 shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-[11px] font-bold text-gray-800 leading-tight">{bid.carName}</p>
                  <p className="text-[10px] text-gray-400 mt-1">Auction: {bid.auctionDate}</p>
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="text-white bg-gray-900">
                {['Date', 'Transaction ID', 'Type', 'Description', 'Ship ID', 'Amount', 'Status'].map((h) => (
                  <th key={h} className="px-4 py-3 text-left font-medium text-[12px] tracking-wide">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {TRANSACTIONS.map((tx, i) => (
                <tr key={tx.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                  ...
                  <td className={`px-4 py-3 font-semibold ${STATUS_CLASSES[tx.status]}`}>{tx.status}</td>
                </tr>
              ))}
            </tbody>
          </table>
